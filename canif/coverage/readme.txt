	...
	TOPPRES/A-CANIF 仕様カバレッジチェックツール実行手順

本ドキュメントは，TOPPRES/A-CANIFの仕様カバレッジチェックツールを実行する
手順を示す．
...
┃ ┗ AUTOSAR_SWS_CANInterface_R4.0.3.txt：AUTOSAR仕様書テキスト版(同梱していない)
┃
┗ implementation
   ┣ get_spec_id.rb                     ：A-CANIFのコードから仕様タグを抜き出すスクリプト
...
AUTOSAR CANIFの仕様書(AUTOSAR_SWS_CANInterface.pdf)をダウンロードし，
Adobe Reader等の変換機能を使用して，テキストファイルを作成する．
ファイル名はAUTOSAR_SWS_CANInterface_R4.0.3.txtとして，(1)フォルダ構成
に従って配置する．
※R4.0.3のみ対応
